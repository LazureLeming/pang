...
public class Ball extends GraphicsObject {
	...
	void initialSettings(int ball, int level){
		...
		try{
			FileData config = new FileData("config/levels/level" + Integer.toString(level) + ".txt");
			
			width = Integer.parseInt(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+4));
			height = Integer.parseInt(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+4));
			horizontal_speed = Double.parseDouble(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+3));
			ball_horizontal_direction = Direction.valueOf(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+5));
			split = Integer.parseInt(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+6));
			shoots = Boolean.parseBoolean(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+7));
			
			if(Double.parseDouble(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+1)) >= 1){
				x = (Configuration.width - this.width);
			}
			else{
				x = (Configuration.width - this.width)*Double.parseDouble(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+1));
			}
			
			if(Double.parseDouble(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+2)) >= 1){
				y = (Configuration.height - this.height);
			}
			else{
				y = (Configuration.height - this.height)*Double.parseDouble(config.readOneLine(config.findInFile("$BALL " + Integer.toString(ball))+2));
			}
